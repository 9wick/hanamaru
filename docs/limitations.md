...
| 入門・再利用のサンプルと型の負例 | `tsc -p docs/spec/tsconfig.json` で検証可能 |
...
同じ構造の別オブジェクトの区別はTypeScriptの型だけではできない。
登録の型による制約に加えて、実行時にオブジェクト参照とキーを照合する。

## モックの範囲

オブジェクトのメソッド差し替えに限定する。モジュールモック、クロージャ内部の参照の置換は提供しない。
